 ...
 */
export function FormularioFreteAgregado({ agregados, clientes }: {
  agregados: AgregadoOpcao[]
  clientes: Array<{ id: string; nome: string }>
}) {
  const [agregadoId, setAgregadoId] = useState(agregados[0]?.id ?? '')
  ...
  return (
    <Formulario action={salvarFreteAgregado} voltarPara="/fretes" rotuloSalvar="Lançar frete">
      {(erros) => (
        <>
          <Card className="grid gap-4 p-4 sm:grid-cols-2">
            ...
            >
              <Select name="fluxoFinanceiro" defaultValue="INTERMEDIADO" required>
          ...
          <Card className="grid gap-4 p-4 sm:grid-cols-2">
            ...
            <Campo label="Número da nota fiscal" erro={erros.numeroNfe}>
              <Input name="numeroNfe" inputMode="numeric" />
            </Campo>
